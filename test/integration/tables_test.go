...
func TestTableStorageEntityOperations(t *testing.T) {
	...
	tableClient := setupTableStorageForEntityOperations(ctx, t, env)
	...
	t.Run("EventLogOperations", func(t *testing.T) {
		...
		eventLogClient := tableClient.NewClient(env.GetResourceNamer().EventLogTableName())
		...
		verifyEventLogEntityFields(t, eventEntity, retrievedEntity)
	})
	...
	t.Run("ResourceRegistryOperations", func(t *testing.T) {
		...
		// Verify entity was written by querying it back
		registryClient := tableClient.NewClient(env.GetResourceNamer().ResourceRegistryTableName())
		...
		// Verify entity fields
		verifyResourceRegistryEntityFields(t, resourceEntity, retrievedEntity)
	})
}
...
func TestTableStorageQueryOperations(t *testing.T) {
	...
	tableClient := setupTableStorageForQueries(ctx, t, env)
	entities := createTestEntities(ctx, t, env)
	eventLogClient := tableClient.NewClient(env.GetResourceNamer().EventLogTableName())

	testAllEntitiesQuery(ctx, t, eventLogClient, entities)
	testFilteredQuery(ctx, t, eventLogClient, entities[0].PartitionKey)
}

func setupTableStorageForQueries(ctx context.Context, t *testing.T, env *test.Environment) *aztables.ServiceClient {
	namer := env.GetResourceNamer()
	storageAccountName := namer.SharedStorageAccountName()
	tableNames := []string{namer.EventLogTableName(), namer.ResourceRegistryTableName()}
...
func createTestEntities(ctx context.Context, t *testing.T, env *test.Environment) []infra.EventLogEntity {
...
func testAllEntitiesQuery(ctx context.Context, t *testing.T, eventLogClient *aztables.Client, entities []infra.EventLogEntity) {
	test.LogTestProgress(t, "testing query operations")

	partitionKey := entities[0].PartitionKey
	retrievedEntities := queryEntitiesByPartition(ctx, t, eventLogClient, partitionKey)

	if len(retrievedEntities) != 3 {
		t.Errorf("should retrieve all 3 entities, got %d", len(retrievedEntities))
	}

	verifyRetrievedEntities(t, entities, retrievedEntities)
}

func queryEntitiesByPartition(ctx context.Context, t *testing.T, eventLogClient *aztables.Client, partitionKey string) []infra.EventLogEntity {
...
func verifyRetrievedEntities(t *testing.T, originalEntities, retrievedEntities []infra.EventLogEntity) {
	...
	for _, originalEntity := range originalEntities {
		...
		verifyEntityFields(t, originalEntity, retrieved)
	}
}

func verifyEntityFields(t *testing.T, original, retrieved infra.EventLogEntity) {
	if original.EventType != retrieved.EventType {
		t.Errorf("event type should match for %s: expected %s, got %s", original.RowKey, original.EventType, retrieved.EventType)
	}
	if original.SessionID != retrieved.SessionID {
		t.Errorf("session ID should match for %s: expected %s, got %s", original.RowKey, original.SessionID, retrieved.SessionID)
	}
	if original.BoxID != retrieved.BoxID {
		t.Errorf("box ID should match for %s: expected %s, got %s", original.RowKey, original.BoxID, retrieved.BoxID)
	}
}

func testFilteredQuery(ctx context.Context, t *testing.T, eventLogClient *aztables.Client, partitionKey string) {
	test.LogTestProgress(t, "testing filtered query operations")

	sessionEntities := queryEntitiesBySession(ctx, t, eventLogClient, partitionKey, "session-1")
...
func queryEntitiesBySession(ctx context.Context, t *testing.T, eventLogClient *aztables.Client, partitionKey, sessionID string) []infra.EventLogEntity {
	sessionFilter := fmt.Sprintf("PartitionKey eq '%s' and SessionID eq '%s'", partitionKey, sessionID)
...
func setupTableStorageForEntityOperations(ctx context.Context, t *testing.T, env *test.Environment) *aztables.ServiceClient {
	namer := env.GetResourceNamer()
	storageAccountName := namer.SharedStorageAccountName()
	tableNames := []string{namer.EventLogTableName(), namer.ResourceRegistryTableName()}
...
func verifyEventLogEntityFields(t *testing.T, expected, actual infra.EventLogEntity) {
	fields := []struct {
		name     string
		expected string
		actual   string
	}{
		{"partition key", expected.PartitionKey, actual.PartitionKey},
		{"row key", expected.RowKey, actual.RowKey},
		{"event type", expected.EventType, actual.EventType},
		{"session ID", expected.SessionID, actual.SessionID},
		{"box ID", expected.BoxID, actual.BoxID},
		{"user key", expected.UserKey, actual.UserKey},
		{"details", expected.Details, actual.Details},
	}

	for _, field := range fields {
		if field.expected != field.actual {
			t.Errorf("%s should match: expected %s, got %s", field.name, field.expected, field.actual)
		}
	}
}

func verifyResourceRegistryEntityFields(t *testing.T, expected, actual infra.ResourceRegistryEntity) {
	fields := []struct {
		name     string
		expected string
		actual   string
		...
		{"VM name", expected.VMName, actual.VMName},
